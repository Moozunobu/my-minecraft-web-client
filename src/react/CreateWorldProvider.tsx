...
export default () => {
  ...
  if (activeCreate) {
    ...
    return <CreateWorld
      ...
      createClick={async () => {
        // create new world
        const { title, type, version, gameMode, plugins, seed, saveType } = creatingWorldState
        ...
        // Build seed value: use provided string as-is (server handles parsing)
        const seedValue = seed.trim() !== '' ? seed.trim() : undefined
        ...
        window.dispatchEvent(new CustomEvent('singleplayer', {
          detail: {
            ...
            version,
            generation: {
              name: 'mountain', // 強制的に新しい山岳地形にする
              options: {
                seed: seedValue || String(Date.now()),
                worldHeight: 256,
                minY: 0
              }
            },
