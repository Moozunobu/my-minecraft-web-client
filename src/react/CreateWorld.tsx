...
const CreateWorldBase = ({ cancelClick, createClick, customizeClick, versions, defaultVersion }) => {
  ...
  const { title, type, version, gameMode, plugins, seed, saveType } = useSnapshot(creatingWorldState)
  useEffect(() => {
  ...
  return <Screen title="Create world" backdrop="dirt">
    ...
    <div style={{ display: 'flex' }}>
      ...
      <Button
        onClick={() => {
          const index = saveTypes.indexOf(saveType)
          creatingWorldState.saveType = saveTypes[index === saveTypes.length - 1 ? 0 : index + 1]
        }}
      >
        Save: {saveType}
      </Button>
